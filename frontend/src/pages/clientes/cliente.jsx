...
import {
  ...
  CircularProgress, Chip, MenuItem, Divider, IconButton,
...
const initialAnimalForm = {
  ...
  dataNascimento: '', alergias: '', observacoes: '',
};
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
      {steps.map((step, i) => {
        ...
        const isAtivo     = step.key === passo || (passo === 'animal_extra' && step.key === 'concluido');
        const isConcluido = stepIdx < idx && !(passo === 'animal_extra' && step.key === 'concluido');
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2 }}>
        <TextField label="Nome completo" name="nome" value={form.nome}
          onChange={(e) => set('nome', e.target.value)} required fullWidth placeholder="Ex: João Silva" />
        <TextField label="Email" name="email" type="email" value={form.email}
          onChange={(e) => set('email', e.target.value)} required fullWidth
          placeholder="Ex: joao.silva@email.com" helperText="Será usado para acesso ao sistema." />
        <TextField label="Telefone" name="telefone" value={form.telefone}
          onChange={(e) => set('telefone', e.target.value.replace(/\D/g, '').slice(0, 15))}
          required fullWidth placeholder="Ex: 910000000" />
        <TextField label="NIF (opcional)" name="nif" value={form.nif}
          onChange={(e) => set('nif', e.target.value.replace(/\D/g, '').slice(0, 9))}
          fullWidth inputProps={{ maxLength: 9 }} placeholder="Ex: 123456789" helperText="9 dígitos numéricos." />
        <TextField label="Morada (opcional)" name="morada" value={form.morada}
          onChange={(e) => set('morada', e.target.value)}
          fullWidth placeholder="Ex: Rua das Flores, 10, Lisboa"
        ...
        <TextField label="Password" name="password"
          ...
          required fullWidth helperText="Mínimo 8 caracteres."
          InputProps={{ endAdornment: eyeBtn(showPwd, () => setShowPwd((v) => !v)) }} />
        <TextField label="Confirmar password" name="confirmarPassword"
          type={showConf ? 'text' : 'password'} value={form.confirmarPassword}
          onChange={(e) => set('confirmarPassword', e.target.value)}
          required fullWidth
          InputProps={{ endAdornment: eyeBtn(showConf, () => setShowConf((v) => !v)) }} />
      </Box>

      <Button type="submit" variant="contained" disabled={loading}
        sx={{ mt: 1, py: 1.5, alignSelf: 'flex-start', backgroundColor: colors.primary,
          '&:hover': { backgroundColor: `${colors.primary}dd` } }}>
        {loading ? <CircularProgress size={22} sx={{ color: '#fff' }} /> : 'Continuar para o Animal →'}
      </Button>
    </Box>
  );
}
...
function AnimalForm({ clienteId, clienteNome, endpoint, onAnimalCriado, onCancelar, isFirst = false, submitLabel }) {
  const { colors } = useThemeContext();
    ...
    <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      {isFirst && (
        <Alert severity="warning" icon={<WarningAmberIcon />}>
          É obrigatório registar pelo menos um animal para <strong>{clienteNome}</strong>.
          O cliente só fica ativo após este passo.
        </Alert>
      )}

      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2 }}>
        <TextField label="Nome do animal" name="nomeAnimal" value={form.nome}
          onChange={(e) => set('nome', e.target.value)} required fullWidth placeholder="Ex: Rex" />
        <TextField label="Espécie" name="especie" value={form.especie}
          onChange={(e) => set('especie', e.target.value)} required fullWidth placeholder="Ex: Cão, Gato..." />
        <TextField label="Raça (opcional)" name="raca" value={form.raca}
          onChange={(e) => set('raca', e.target.value)} fullWidth placeholder="Ex: Labrador" />
        <TextField select label="Porte" name="porte" value={form.porte}
          onChange={(e) => set('porte', e.target.value)} required fullWidth>
          {PORTE_OPTIONS.map((o) => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}
        </TextField>
        <TextField label="Data de nascimento (opcional)" name="dataNascimento"
          type="date" value={form.dataNascimento}
          onChange={(e) => set('dataNascimento', e.target.value)}
          fullWidth InputLabelProps={{ shrink: true }} />
        <TextField label="Alergias (opcional)" name="alergias" value={form.alergias}
          onChange={(e) => set('alergias', e.target.value)}
          fullWidth placeholder="Ex: Pólen, determinados champôs..." />
        <TextField label="Observações (opcional)" name="observacoes" value={form.observacoes}
          onChange={(e) => set('observacoes', e.target.value)}
          fullWidth multiline minRows={2} placeholder="Informações adicionais relevantes..."
          sx={{ gridColumn: { md: '1 / -1' } }} />
      </Box>

      <Box sx={{ display: 'flex', gap: 2, mt: 1, flexWrap: 'wrap' }}>
        <Button type="submit" variant="contained" disabled={loading}
          startIcon={loading ? null : (isFirst ? <CheckCircleOutlineIcon /> : <AddIcon />)}
          sx={{ py: 1.5, backgroundColor: colors.primary,
            '&:hover': { backgroundColor: `${colors.primary}dd` } }}>
          {loading
        ...
        {onCancelar && (
          <Button variant="outlined" onClick={onCancelar} sx={{ py: 1.5 }}>Cancelar</Button>
        )}
      </Box>
    </Box>
  );
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <Box sx={{ flex: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5, flexWrap: 'wrap' }}>
            ...
            <Typography variant="subtitle1" sx={{ fontWeight: 700, color: colors.text }}>{cliente.nome}</Typography>
            <Chip size="small" label={cliente.ativo ? 'Ativo' : 'Inativo'}
              color={cliente.ativo ? 'success' : 'default'} sx={{ fontSize: '11px', height: 22 }} />
            {(cliente.animais?.length ?? 0) > 0 && (
              <Chip size="small"
                ...
                variant="outlined" color="primary" sx={{ fontSize: '11px', height: 22 }} />
            )}
          </Box>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', ml: 1 }}>
          <Button size="small" variant="outlined" startIcon={<AddIcon />}
            onClick={() => onAddAnimal(cliente)} sx={{ fontSize: '12px', whiteSpace: 'nowrap' }}>
            Animal
          </Button>
          {(cliente.animais?.length ?? 0) > 0 && (
      ...
      <Collapse in={expanded}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {(cliente.animais || []).map((a) => (
            <Box key={a.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', pl: 0.5 }}>
              <PetsIcon sx={{ fontSize: 16, color: colors.primary }} />
              <Typography variant="body2" sx={{ fontWeight: 600, color: colors.text }}>{a.nome}</Typography>
              <Typography variant="body2" sx={{ color: colors.textSecondary }}>
                {a.especie}{a.raca ? ` · ${a.raca}` : ''}
              </Typography>
              <Chip size="small" label={porteLabel(a.porte)} variant="outlined" sx={{ fontSize: '10px', height: 20 }} />
              {a.dataNascimento && (
                <Typography variant="body2" sx={{ color: colors.textSecondary, fontSize: '11px' }}>
...
export default function CreateClient() {
  ...
  const handleCancelarNoAnimal = async () => {
    if (clienteId) {
      try { await fetch(`${API_BASE_URL}/clientes/${clienteId}`, { method: 'DELETE' }); }
      catch { /* job de limpeza trata depois */ }
    }
  ...
  const handleAnimalAdicionadoAClienteExistente = (a) => {
    setMensagemSucesso(`Animal "${a.nome}" adicionado a "${clienteParaAnimal.nome}" com sucesso!`);
  ...
  return (
    <Box>
      <Typography variant="h1" sx={{ mb: 1, color: colors.text }}>Registo de Clientes</Typography>
      <Typography variant="body1" sx={{ mb: 4, color: colors.textSecondary }}>
        Registo interno de novos clientes com os seus animais.
        O cliente só fica ativo após registar pelo menos um animal.
      </Typography>

      {/* ── Painel: adicionar animal a cliente existente ── */}
      {clienteParaAnimal && (
        <Paper elevation={2} sx={{ borderRadius: 3, p: 3, mb: 4, border: `2px solid ${colors.primary}` }}>
      ...
      {mensagemSucesso && passo === 'cliente' && !clienteParaAnimal && (
        <Alert severity="success" sx={{ mb: 3 }} onClose={() => setMensagemSucesso('')}>
          {mensagemSucesso}
        </Alert>
      )}
        ...
        <Paper elevation={2} sx={{ borderRadius: 3, p: 3, mb: 4 }}>
          ...
          {passo === 'cliente' && (
            <>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                ...
                <Typography variant="h2" sx={{ color: colors.text }}>Dados do Cliente</Typography>
              </Box>
          ...
          {passo === 'animal' && clienteAtual && (
            <>
              <Alert severity="success" sx={{ mb: 2 }}>
                Dados de <strong>"{clienteNome}"</strong> guardados temporariamente.
                Registe agora o primeiro animal para confirmar o registo.
              </Alert>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                <PetsIcon sx={{ color: colors.primary }} />
                <Typography variant="h2" sx={{ color: colors.text }}>Primeiro Animal</Typography>
              </Box>
              <AnimalForm
                clienteId={clienteId}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ p: 2, backgroundColor: '#f6fbf6', borderRadius: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  ...
                  <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{clienteNome}</Typography>
                  <Typography variant="body2" sx={{ color: colors.textSecondary }}>{clienteEmail}</Typography>
                </Box>
                {animaisRegistados.map((a) => (
                  <Box key={a.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, pl: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                <Button variant="outlined" startIcon={<AddIcon />}
                  onClick={() => { setMensagemSucesso(''); setPasso('animal_extra'); }}>
                  Adicionar Outro Animal
                </Button>
                <Button variant="contained" onClick={handleNovoRegisto}
          ...
          {passo === 'animal_extra' && clienteAtual && (
            <>
              ...
              {mensagemSucesso && (
                <Alert severity="success" sx={{ mb: 2 }}>{mensagemSucesso}</Alert>
              )}
              <AnimalForm
                clienteId={clienteId}
                clienteNome={clienteNome}
      ...
      <Paper elevation={2} sx={{ borderRadius: 3, p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h2" sx={{ color: colors.text }}>Clientes Registados</Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {clientes.map((c) => (
            <ClienteCard key={c.id} cliente={c} onAddAnimal={handleAddAnimalAClienteExistente} />
          ))}
        </Box>
      </Paper>
    </Box>
