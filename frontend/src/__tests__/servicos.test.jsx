...
function mockDefaultFetch(
  servicosList = [SERVICO_ATIVO_MOCK],
  regrasList = [REGRA_MOCK],
) {
  global.fetch
...
describe("ServicosPage — carregamento inicial", () => {
  ...
  test("faz dois pedidos no mount — /servicos e /regras-preco", async () => {
    ...
    expect(global.fetch).toHaveBeenNthCalledWith(
      2,
      "http://localhost:5000/regras-preco",
    );
  });
...
describe("ServicosPage — validação do formulário", () => {
  ...
  test("mostra erro quando nome está vazio", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /Criar Serviço/i }),
    );
    expect(
      await screen.findByText("O nome do serviço é obrigatório."),
    ).toBeInTheDocument();
    expect(global.fetch).toHaveBeenCalledTimes(2);
  });
  ...
  test("não submete quando preço base é zero", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText(/Nome do serviço/i),
      "Novo servico",
    );
...
describe("ServicosPage — criar serviço", () => {
  ...
  test("submete com preço único e envia 5 regras iguais para todos os portes", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Serviço criado com sucesso."),
      ).toBeInTheDocument(),
    );
  ...
  test("submete com preço por porte e envia 5 regras com preços distintos", async () => {
    ...
    portes.forEach((p, i) =>
      global.fetch.mockImplementationOnce(() =>
        mockJsonResponse({
          id: `r-${i}`,
          tipoServicoId: "srv-porte",
          porteAnimal: p,
          precoBase: precos[i],
          duracaoMinutos: duracoes[i],
        }),
      ),
    );
    global.fetch
    ...
    await userEvent.click(
      screen.getByRole("switch", { name: /Preço por porte/i }),
    );
    ...
    for (let i = 0; i < portes.length; i++) {
      ...
      fireEvent.change(inputs[i * 2 + 1], {
        target: { value: String(duracoes[i]) },
      });
    }
    await userEvent.click(
      screen.getByRole("button", { name: /Criar Serviço/i }),
...
describe("ServicosPage — editar serviço", () => {
  ...
  test("ao entrar em modo edição aparece nota informativa sobre histórico", async () => {
    ...
    expect(
      await screen.findByText(
        /As alterações às regras de preço aplicam-se apenas a/i,
      ),
    ).toBeInTheDocument();
  ...
  test("guardar edição abre diálogo de confirmação antes de submeter", async () => {
    ...
    await screen.findByText("Editar Serviço");
    await userEvent.click(
      screen.getByRole("button", { name: /Atualizar Serviço/i }),
    );
  ...
  test("confirmar edição chama PUT e mostra mensagem de sucesso", async () => {
    const servicoAtualizado = {
      ...SERVICO_ATIVO_MOCK,
      tipo: "Corte de unhas actualizado",
    };
    ...
    global.fetch
      ...
      .mockImplementationOnce(() =>
        mockJsonResponse({ ...servicoAtualizado, regrasPreco: [REGRA_MOCK] }),
      )
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Serviço atualizado com sucesso!"),
      ).toBeInTheDocument(),
    );
...
describe("ServicosPage — inativar serviço", () => {
  ...
  test("API devolve 409 — mostra mensagem de bloqueio por agendamentos futuros", async () => {
    const msgErro =
      'Não é possível inativar o serviço "Corte de unhas" porque tem 2 agendamento(s) futuro(s) associado(s). Cancele os agendamentos antes de inativar o serviço.';

    global.fetch
      .mockImplementationOnce(() => mockJsonResponse([SERVICO_ATIVO_MOCK]))
      .mockImplementationOnce(() => mockJsonResponse([REGRA_MOCK]))
      .mockImplementationOnce(() =>
        mockJsonResponse({ error: msgErro }, false, 409),
      );
  ...
  test("API devolve 409 — serviço mantém-se ativo na lista", async () => {
    const msgErro =
      'Não é possível inativar o serviço "Corte de unhas" porque tem 1 agendamento(s) futuro(s) associado(s).';
  ...
  test("API devolve 500 — mostra mensagem de erro vinda da API", async () => {
    global.fetch
      ...
      .mockImplementationOnce(() => mockJsonResponse([REGRA_MOCK]))
      .mockImplementationOnce(() =>
        mockJsonResponse({ error: "Erro interno" }, false, 500),
      );
...
describe("ServicosPage — reativar serviço", () => {
  ...
  test("ao editar serviço inativo aparece botão Reativar Serviço", async () => {
    ...
    await screen.findByText("Editar Serviço");
    expect(
      screen.getByRole("button", { name: /Reativar Serviço/i }),
    ).toBeInTheDocument();
  });
  ...
  test("confirmar reativação chama POST /reativar e mostra sucesso", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Serviço reativado com sucesso!"),
      ).toBeInTheDocument(),
    );

    const postCall = global.fetch.mock.calls[2];
    expect(postCall[0]).toContain(
      `/servicos/${SERVICO_INATIVO_MOCK.id}/reativar`,
    );
...
describe("ServicosPage — chips de preço na listagem", () => {
  ...
  test('mostra chip "Preço único" quando todos os preços das regras são iguais', async () => {
    ...
    const listaServicos = document.querySelector(
      ".MuiPaper-elevation2:last-of-type",
    );
  ...
  test('mostra chip "Preço por porte" quando os preços das regras são diferentes', async () => {
    global.fetch
      .mockImplementationOnce(() =>
        mockJsonResponse([
          { id: "srv-3", tipo: "Banho completo", ativo: true },
        ]),
      )
