...
describe("Salas page", () => {
  ...
  test("carrega servicos e salas no mount", async () => {
    ...
    expect(global.fetch).toHaveBeenNthCalledWith(
      2,
      "http://localhost:5000/salas/todas",
    );
  });
  ...
  test("mostra mensagem quando nao existem salas", async () => {
    ...
    expect(
      await screen.findByText(/Ainda não existem salas registadas/i),
    ).toBeInTheDocument();
  });
  ...
  test("mostra erro quando nome está vazio", async () => {
    ...
    expect(
      await screen.findByText("Nome da sala é obrigatório."),
    ).toBeInTheDocument();
  });
  ...
  test("mostra erro quando capacidade é negativa", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Preço por hora/i), {
      target: { value: "10" },
    });
    ...
    expect(
      await screen.findByText(
        "Capacidade deve ser um número positivo maior que zero.",
      ),
    ).toBeInTheDocument();
  });
  ...
  test("mostra erro quando preço é negativo", async () => {
    ...
    expect(
      await screen.findByText(
        "Preço por hora deve ser um valor positivo maior que zero.",
      ),
    ).toBeInTheDocument();
  });
  ...
  test("mostra erro quando nenhum serviço está selecionado", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "Selecione pelo menos um serviço para a sala.",
    );
  });
  ...
  test("mostra erro 409 da API para nome duplicado", async () => {
    global.fetch
      ...
      .mockImplementationOnce(() =>
        mockJsonResponse(
          { error: 'Ja existe uma sala com o nome "Sala Nova".' },
          false,
          409,
        ),
      );
  ...
  test("clicar em editar preenche o formulário com dados da sala", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByDisplayValue(SALA_ATIVA_MOCK.equipamento),
      ).toBeInTheDocument();
  ...
  test("submit em modo edição chama PUT com payload correto", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Sala atualizada com sucesso!"),
      ).toBeInTheDocument();
    });
  ...
  test("reativar sala inativa através do botão no formulário de edição", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /Reativar Sala/i }),
    );

    await waitFor(() => {
      expect(
        screen.getByText("Sala reativada com sucesso!"),
      ).toBeInTheDocument();
    });
