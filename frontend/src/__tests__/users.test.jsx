...
describe('Users page', () => {
  ...
  test('carrega servicos e funcionarios no mount', async () => {
    global.fetch
      .mockImplementationOnce(() => mockJsonResponse([{ id: 'srv-1', tipo: 'BANHO' }]))
    ...
    expect(global.fetch).toHaveBeenNthCalledWith(2, 'http://localhost:5000/funcionarios');
  });
  ...
  test('mostra erro de validacao quando horaInicio >= horaFim', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/Nome completo/i), 'Teste Horario');
    ...
    fireEvent.change(screen.getByLabelText(/Hora inicio/i), { target: { value: '18:00' } });
    fireEvent.change(screen.getByLabelText(/Hora fim/i), { target: { value: '09:00' } });

    await userEvent.click(screen.getByRole('button', { name: /Criar Funcionario/i }));

    expect(await screen.findByText('horaInicio deve ser menor que horaFim.')).toBeInTheDocument();
    expect(global.fetch).toHaveBeenCalledTimes(2);
  });

  test('submete com sucesso e envia payload correto', async () => {
    global.fetch
      .mockImplementationOnce(() => mockJsonResponse([{ id: '11111111-1111-4111-8111-111111111111', tipo: 'BANHO' }]))
      .mockImplementationOnce(() => mockJsonResponse([]))
      .mockImplementationOnce(() =>
        mockJsonResponse({
          id: 'f-1',
        ...
        mockJsonResponse([
          {
            ...
            ativo: true,
            horariosTrabalho: [{ diasSemana: ['TERCA'] }],
            servicos: [{ tipoServicoId: '11111111-1111-4111-8111-111111111111', tipo: 'BANHO' }],
          },
        ])
      );
    ...
    await userEvent.type(screen.getByLabelText(/Nome completo/i), 'Novo Funcionario');
    await selectCargo('Banhista');
    ...
    await waitFor(() => {
      expect(screen.getByText('Funcionario criado com sucesso.')).toBeInTheDocument();
    });
    ...
    expect(payload.tipoServicoIds).toEqual(['11111111-1111-4111-8111-111111111111']);
  ...
  test('mostra erro vindo da API no submit', async () => {
    global.fetch
      .mockImplementationOnce(() => mockJsonResponse([{ id: '11111111-1111-4111-8111-111111111111', tipo: 'BANHO' }]))
      .mockImplementationOnce(() => mockJsonResponse([]))
      .mockImplementationOnce(() => mockJsonResponse({ error: 'Ja existe um funcionario com o email "x@x.com".' }, false, 409));

    renderUsers();
    await screen.findByText('Gestao de Funcionarios');

    await userEvent.type(screen.getByLabelText(/Nome completo/i), 'Email Duplicado');
    await selectCargo('Banhista');
    ...
    expect(await screen.findByText('Ja existe um funcionario com o email "x@x.com".')).toBeInTheDocument();
  });
});
